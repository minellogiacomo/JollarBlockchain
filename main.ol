...
init {
  ...
  if (global.status.createGenesisBlock == true) {
   ...
   for ( i=1, i<#global.peertable.node, i++ ) {
     ...
     //TO DO: OPTIONAL add validation with blockVerification@blockInternalVerification(currentblock)(blockVerificationResponse);
  ...
  [DemoTx(TxValue)(DemoTXResponse) {
   ...
   for ( i = 0, i < #global.peertable.node, i++  ){
     if (global.peertable.node[i].location==TxValue.location){
      ...
      sum=0;
      //TO DO change blockcahin navigation, use previous block hash
    ...
    println@Console( "Send md5 hash request" )();
    md5@MessageDigest(block.previousBlockHash+ //TO DO:better define order and what to hash
                      block.size+
                      block.version+
                      block.n+
                      block.time+
